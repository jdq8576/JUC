...
public class Test2AtomicInteger {
    public static void main(String[] args) {
         ...
         */
        System.out.println(i.getAndDecrement()); // 0

        // 获取并加值（i = 0, 结果 i = 5, 返回 0）
        System.out.println(i.getAndAdd(5)); // -1

        // 加值并获取（i = 5, 结果 i = 0, 返回 0）
        System.out.println(i.addAndGet(-5)); // -1

        // 获取并更新（i = 0, p 为 i 的当前值, 结果 i = -2, 返回 0）
        // 其中函数中的操作能保证原子，但函数需要无副作用
        System.out.println(i.getAndUpdate((p) -> {
            return p + 2;
        })); // -1

        // 更新并获取（i = -2, p 为 i 的当前值, 结果 i = 0, 返回 0）
        // 其中函数中的操作能保证原子，但函数需要无副作用
        System.out.println(i.updateAndGet(p -> p + 2)); // 3
        ...
        System.out.println(i.getAndAccumulate(20, (p, x) -> p + x));
